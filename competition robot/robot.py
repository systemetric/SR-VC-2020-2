...
num_primary = len(main_cubes[R.zone])
while True and iteration < num_primary + len(secondary_cubes[R.zone]):
    ...
    while not R.have_cube:
        if iteration < num_primary:
            ...
            R.drive_to_marker(target, 0.4, 80, 0.2, 3, 5, True)
            R.turn_to_marker(target, 0.2, True)
        else:
            R.seek_markers_with_priority([target], mid_term_markers[R.zone][iteration - num_primary])
            R.drive_to_markers_with_priority([target], mid_term_markers[R.zone][iteration - num_primary], 1.4, 80, 0.2, 3, 5, True)
            R.seek_marker(target)
            R.drive_to_marker(target, 0.4, 80, 0.2, 3, 5, True)
            R.turn_to_marker(target, 0.2, True)
        R.setDriveMotors(50)
    ...
    if iteration < num_primary:
        ...
        R.drive_to_markers(return_markers[R.zone], return_marker_distance, 80, 0.2, 3, 20, True)
        R.openGripper()
        R.setDriveMotors(-20)
        R.sleep(0.2)
        R.stopDriveMotors()
        ...
        R.drive_to_markers_with_priority(return_markers[R.zone], mid_term_markers[R.zone][iteration - num_primary], 1.4, 80, 0.2, 3, 20, True)
        R.seek_markers(return_markers[R.zone], 10, 10)
        R.drive_to_markers(return_markers[R.zone], return_marker_distance, 80, 0.2, 3, 20, True)
        R.openGripper()
        R.setDriveMotors(-60)
