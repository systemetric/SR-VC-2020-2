...
while True and iteration < num_primary + len(secondary_cubes[R.zone]):
    ...
    while not R.have_cube:
        if iteration < num_primary:
            ...
            R.drive_to_markers_with_priority([target], mid_term_markers[R.zone][iteration - num_primary], 1.5, 80)
            R.seek_marker(target)
            R.drive_to_marker(target, 0.4, 80)
            R.turn_to_marker(target, 0.2, True)
        R.setDriveMotors(50)
    ...
    if iteration < num_primary:
        ...
        R.drive_to_markers_with_priority(return_markers[R.zone], mid_term_markers[R.zone][iteration - num_primary], 1.5, 80, 0.2, 3, 20)
        R.seek_markers(return_markers[R.zone], 10, 10)
        R.drive_to_markers(return_markers[R.zone], 1.3, 80, 0.2, 3, 20)
        R.openGripper()
        R.setDriveMotors(-60)
