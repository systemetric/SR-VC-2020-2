...
return_marker_distance = 0.9
cube_ramming_distance = 0.4
move_speed = 60
...
while True and iteration < num_primary + len(secondary_cubes[R.zone]):
    ...
    while not R.have_cube:
        if iteration < num_primary:
            ...
            R.drive_to_marker(target, cube_ramming_distance, move_speed, 0.2, 3, 5, False)
            R.turn_to_marker(target, 0.2, True)
        else:
            R.seek_markers_with_priority([target], mid_term_markers[R.zone][iteration - num_primary])
            R.drive_to_markers_with_priority([target], mid_term_markers[R.zone][iteration - num_primary], 1.4, move_speed, 0.2, 3, 5, False)
            R.seek_marker(target)
            R.drive_to_marker(target, cube_ramming_distance, move_speed, 0.2, 3, 5, False)
            R.turn_to_marker(target, 0.2, True)
        R.setDriveMotors(50)
    ...
    if iteration < num_primary:
        ...
        R.drive_to_markers(return_markers[R.zone], return_marker_distance, move_speed, 0.2, 3, 20, False)
        R.openGripper()
        R.setDriveMotors(-40)
        R.sleep(0.5)
        R.stopDriveMotors()
        ...
        R.drive_to_markers_with_priority(return_markers[R.zone], mid_term_markers[R.zone][iteration - num_primary], 1.4, move_speed, 0.2, 3, 20, False)
        R.seek_markers(return_markers[R.zone], 10, 10)
        R.drive_to_markers(return_markers[R.zone], return_marker_distance, move_speed, 0.2, 3, 20, False)
        R.openGripper()
        R.setDriveMotors(-40)
